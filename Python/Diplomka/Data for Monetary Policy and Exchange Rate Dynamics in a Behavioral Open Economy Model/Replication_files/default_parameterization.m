function params = default_parameterization(inputs)

%Inputs
params.s_1 = inputs(1); %Takes on values 0 or 1:   Set to zero to switch off extra terms in IS curve
params.s_2 = inputs(2); %Takes on values 0 or 1:   Set to zero to switch off discounting of inflation in real rate definition
params.s_3 = inputs(3); %Takes on values 0 or 1:   Set to zero to impose closed economy version of the model
params.s_4 = inputs(4); %Takes on values 0 or 1:   Set to zero to shock real rate directly (nominal rate otherwise)
params.s_5 = inputs(5); %Takes on values 0 or 1:   Set to zero to disable all shocks other than domestic monetary policy shocks

%This is the default parameterization of the OE_BNK model of Kolasa, Ravgotra and Zabczyk

%This version 11/9/2021
...
params.m                = 0.85;                 % Cognitive Discounting  (Gabaix, 2020; Table 1)
params.m_star           = params.m;             % Set discounting equal in both economies unless specified otherwise
...
params.rho_a            = 0.95;                 % Autocorrelation Technology Shock                                               
params.rho_a_star       = params.rho_a;         % Autocorrelation Technology Shock                                               
...
%Set size to zero in closed economy case (do nothing otherwise)
params.alppha           = params.s_3*params.alppha;
