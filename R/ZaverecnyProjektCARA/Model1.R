...
ZahranicniObchod <- read_excel("data.xlsx", 
...
print(CARA)
ggsave(filename = "grafy/vsechny_CARA1.png", plot = CARA, width = 13, 
       height = 10, dpi = 300)
...
adf_results
write.table(adf_results, file = "tabulky/vysledky_ADF_testu1.txt", sep = "\t", 
            row.names = FALSE, quote = FALSE)

# Vykreslení transformovaných proměnných #######################################

data_long <- data %>%
  dplyr::select(date, df_l_y_ger, df_l_y_us, df_l_z, df_l_X) %>%
  pivot_longer(
    cols = -date,
    names_to = "variable",
    values_to = "value"
...
print(CARA_diff)
ggsave(filename = "grafy/transformovane_CARA1.png", plot = CARA_diff, 
...
print(CARA_gap)
ggsave(filename = "grafy/gap_CARA1.png", plot = CARA_gap, width = 13, 
       height = 10, dpi = 300)
...
corr_diff <- ggplot(lagged_correlations, aes(x = var2, y = var1, 
  ...
  facet_wrap(~ lag) +
  scale_fill_gradient2(high = "#2B645E", mid = "#f5f5f5", low = "#84541E",
...
print(corr_diff)
ggsave(filename = "grafy/korelogram_diff1.png", plot = corr_diff, 
       width = 13, height = 10, dpi = 300)
...
print(corr_gap)
ggsave(filename = "grafy/korelogram_gap1.png", plot = corr_gap,
       width = 13, height = 10, dpi = 300)
...
sink("tabulky/Diff_VAR_modely1.txt")
cat("--------------------------------------------------\n")
...
print(IRF_diff_var)
ggsave(filename = "grafy/IRF_diff_VAR1.png", plot = IRF_diff_var, 
       width = 13, height = 10, dpi = 300)
...
sink("tabulky/Gap_VAR_modely1.txt")
cat("--------------------------------------------------\n")
...
print(IRF_gap_var)
ggsave(filename = "grafy/IRF_gap_VAR1.png", plot = IRF_gap_var, 
       width = 13, height = 10, dpi = 300)
...
ggsave(filename = "grafy/forecast1.png", plot = combined_forecast, 
       width = 13, height = 10, dpi = 300)
...
sink("tabulky/forecast_modelu1.txt", append = TRUE)
cat("--------------------------------------------------\n")
